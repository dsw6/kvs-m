...
const ERRORS = {
   ...
   NO_DEL:     3       // item cannot be deleted
};
...
function createStore(opts)
{
   ...
   return({insert: insertFunc, upsert: upsertFunc, get: getFunc, del: delFunc, clear: clearFunc, metrics: metricsFunc, info: infoFunc});
   ...
   function insertFunc(key, value, type)
   {
      ...
      if ( _store.size >= _store.maxSize )
         return(new KVSError(ERRORS.MAX_SIZE, "insert failed, max size reached"));

      if ( _store.items[key] )
         return(new KVSError(ERRORS.EXISTS, "insert failed, key already exists"));
   ...
   function getFunc(key)
   {
      ...
      if ( !item || (item.exp < now) )
         return( {err: new KVSError(ERRORS.NOT_FOUND, "item not found"), value: undefined, type: undefined} );

      item.exp = now + _store.itemTTL;
      return( {err: undefined, value: item.value, type: item.type} );
   }
   ...
   function delFunc(key)
   {
      ...
      if ( !item || (item.exp < Date.now()) )
        return(new KVSError(ERRORS.NOT_FOUND, "item not found"));

      if (ITEM_TYPE.PERM === item.type)
         return(new KVSError(ERRORS.NO_DEL, "cannot delete item"));
